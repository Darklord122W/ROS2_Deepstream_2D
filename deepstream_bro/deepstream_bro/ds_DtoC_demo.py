...
class DeepStreamPgieSgiePublisher(Node):
    ...
    def _publish_annotated(self, msg: Image):
        with self._det_lock:
            dets = list(self._last_ann_dets)

        # If no detections yet, still republish input as image topic.
        if not dets:
        ...
        for (x1, y1, x2, y2, det_label, det_score, cls_label, cls_score) in dets:
    ...
    def _on_buffer_probe(self, pad, info, _):
        ...
        while l_frame is not None:
            ...
            while l_obj is not None:
                ...
                x1 = float(obj.rect_params.left) * sx
                y1 = float(obj.rect_params.top) * sy
                w = float(obj.rect_params.width) * sx
                h = float(obj.rect_params.height) * sy
                ...
                ann_dets.append(
                    (x1, y1, x2, y2, pgie_label, float(obj.confidence), sgie_label_text, sgie_prob)
                )
